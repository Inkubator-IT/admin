...
import type { ClientInformation, CreateClientInformationRequest, UpdateClientInformationRequest } from "./types";
...
class ClientInformationApiService extends ApiClient {
  ...
  async getClientInformationById(id: number): Promise<ApiResponse<ClientInformation>> {
    return this.request<ClientInformation>(`/api/client-information/${id}`);
  }

  async createClientInformation(data: CreateClientInformationRequest): Promise<ApiResponse<ClientInformation>> {
    return this.request<ClientInformation>("/api/client-information", {
      method: "POST",
  ...
  async deleteClientInformation(id: number): Promise<ApiResponse<{ message: string }>> {
